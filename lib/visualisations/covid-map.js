function CovidMap() {
  ...
  this.setup = function () {
    ...
    // Set a callback that will be called when a country area hovered with a mouse
    this.worldMap.onCountryHover = function (countryCode, countryName) {
      // Display a tip with the country name and the statistics values
      fill('black');
  ...
  function updateData() {
    ...
    // fetch data on the specific day
    self.dayData = self.covidData.findRows(self.date, 'Date_reported');

    // CSV field for the selected dataset
    let fieldName = fieldNameMap[self.dataSetSelector.value()];

    // Calulate min and max values for all values in the dataset
