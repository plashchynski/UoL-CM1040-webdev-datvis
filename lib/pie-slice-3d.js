 ...
 */
 PieSlice3D = function (renderer) {
   ...
   * @param {string} colour
   */
  this.draw = function (start, stop, radius, height, colour) {
    this.renderer.noStroke();
    this.renderer.fill(colour);

    buildTopFaceShape(this.renderer, start, stop, radius);
    buildBottomFaceShape(this.renderer, start, stop, radius, height);

    // Darker colour for walls
    let wallColour = changeColourLightness(this.renderer, colour, -10);
    this.renderer.fill(wallColour);

    buildRadialWalls(this.renderer, start, stop, radius, height);
    buildInnerWalls(this.renderer, start, stop, radius, height);
  };
