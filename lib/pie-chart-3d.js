  ...
  this.draw = function(dataPoints, labels, colours, title) {
    ...
    for (var i = 0; i < angles.length; i++) {
      ...
      if (labels) {
        ...
        text(label + ' ' + formatNumber(value) + '%', x + boxWidth + labelsRightMargin, y + boxWidth / 2);

        // Check if label hovered with a mouse
        if (mouseX >= x && mouseX <= (x + boxWidth + labelsRightMargin + label.length * labelsFontSize) &&
      ...
      let segment = new CylinderSegment();
      segment.draw(this.renderer, start, stop, this.radius, height, colour);
    ...
    image(this.renderer, this.x-this.diameter/2, this.y-this.diameter/2);
