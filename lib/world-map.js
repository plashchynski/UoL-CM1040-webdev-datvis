...
function WorldMap() {
   ...
   */
  this.onCountryHover = function (countryCode, countryName) {
    console.debug(`${countryCode} ${countryName} is hovered.`);
  };
  ...
  function checkCountryHover() {
    let hoveredRegion = select('svg>path:hover,svg>g:hover', self.mapDiv);
    if (!hoveredRegion)
      return;

    let countryNameTag = select('name', hoveredRegion);
    let countryCode = hoveredRegion.id();
    if (!countryNameTag || !countryCode)
      return;

    let countryName = countryNameTag.html();
